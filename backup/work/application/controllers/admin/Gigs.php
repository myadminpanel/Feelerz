...
public function get_one_data(){
     ...
     $data=$this->db->query("select * from enquerys where id='".$id."'")->row_array();
     echo @$data["message"];
}
    ...
    public function enquiry()
    {
        ...
        $this->data['page'] = 'enquiry';

        // $this->data['links'] = $this->pagination->create_links();

        // $start = (int)$this->uri->segment(3);

        $this->data['list'] = $this->admin_panel_model->all_enquirys();                 
         $this->db->query("update enquerys SET notification_status=1 where notification_status=0");
        ...
        if($this->input->post('id')){
            ...
            if(!empty($id)){
               ...
               if($count==0){
                 ...
                 $this->db->query("DELETE FROM `crasol` WHERE type='package' and item_id='".$id."'");
    ...
    public function gig_activate(){
        ...
        if($this->input->post('gig_id')){
            ...
            $this->session->set_flashdata('message', 'The package has been activated successfully...');
