...
public function get_one_post_by_id()
{
   ...
   $get_post=$this->db->query("SELECT * FROM post WHERE id='".$post_id."'")->row_array();
...
public function add_feelings()
{
   ...
   {
      echo json_encode(array("status"=>"0","message"=>"all feeled are required"));
   }
    else
    {

      $get_data=$this->db->query("SELECT * FROM feeling WHERE name='".$name."' and parrent='".@$parrent."'")->result_array();

      if(@$get_data)
        {

            $this->db->query("UPDATE feeling SET reating = reating + 1 WHERE name='".$name."'");
            echo json_encode(array("status"=>"1","data"=>@$get_data,"message"=>"feeling insert successfully."));
        }
    else
     {
        $insert_data=array(
      ...
      $this->db->insert("feeling",$insert_data);
       $last_data=$this->db->query("SELECT * FROM feeling WHERE user_id='".@$user_id."' ORDER BY id DESC LIMIT 1")->row_array();
          echo json_encode(array("status"=>"1","data"=>@$last_data,"message"=>"feeling insert successfully."));
 ...
 public function version_control()
  {
      ...
      $data=$this->db->query("SELECT * FROM version_control WHERE version='".$version."' and  status='1' ")->row_array();

      if(@$data)
       {

          echo json_encode(array("status"=>"1","message"=>"version is running"));
       }
        else
        {
...
public function change_login_status()

 {
   ...
   try {
        ...
        echo json_encode(array("status"=>"1","message"=>"Login status chage"));
       ...
       echo json_encode(array("status"=>"0","message"=>"something was wrong pleace try agiam."));
...

 {
   ...
   // var_dump(count($tag_frieds_in_array));
   ...
   $last_tag_id_in_string=implode("','",$new_resent_ids_array);
    // var_dump($last_tag_id_in_string);
       ...
       if($userid!="" && $password!="")
...

        {
...

           {
                   ...
                   echo json_encode(array("status"=>"0","message"=>"account is not activated."));
...

      {
        ...
        if(@$social_id)
...

          {
            ...
            $get_result=$this->apimodel->social_login_dsp($social_id,$insert_data);
...

         {
          ...
          if(@$friend_ids["following"])
          {
            ...
            $following=explode(",", $friend_ids["following"]);
             ...
             $message=$friend_ids["name"]." is add new post";
...
// var_dump($get_data_for_notification);
        // var_dump($this->db->last_query());
 ...
 // var_dump("fbvbgfbg sdcdscds");
              $key=array_search($user_id,$set_friend);
                ...
                $message=$user_name["name"]." unlike on your '".@$get_data["post_text"]."' post.";



                $message_2="unlike on your '".@$get_data["post_text"]."' post.";
...
$message=@$commented_by["name"]." user is commented on '".$user_data["post_text"]."' POST";
$message_2="commented on '".$user_data["post_text"]."' POST";
...
// var_dump($message);
 ...
 $insert_notifiction=array(
             ...
             {
               echo json_encode(array("status"=>"0","message"=>"Comment not uploaded"));
             }
...
// var_dump($message);
  ...
  public function get_comment()
...

   {
       ...
       $get_comment=$this->db->query(" SELECT g.*,u.name ,u.profileimage FROM get_comment as g INNER JOIN users as u ON g.user_id = u.id WHERE g.post_id = '".$post_id."'")->result_array();  
  ...
  //  }
    ...
    public function get_post()

     {
        ...
        $friends=$this->db->query("select following,following from users where id='".$user_id."' ")->row_array();
...
echo json_encode(array("status"=>"1","data"=>$get_follwing_result));
    ...
    public function send_follow()

     {
        ...
        $send_result = $this->db->query("SELECT send_follow,name FROM users WHERE id='".$current_user_id."'")->row_array();



        if($send_result['send_follow'])

         {



            $sent_friend = explode(",", $send_result['send_follow']);

            array_push($sent_friend,$reciever_user_id);
...

         {
            ...
            $send_friend_request=$reciever_user_id;

         }



          $result_update=$this->db->query("update users set send_follow='".$send_friend_request."' where id='".$current_user_id."'"); 



          $get_result = $this->db->query("SELECT get_follow FROM users WHERE id='".$reciever_user_id."'")->row_array();



        if($get_result['get_follow'])

           {

               $get_friend_array = explode(",", $get_result['get_follow']);
            ...
            $get_friend_Result=$this->db->query("update users set get_follow='".$get_friends_string."' where id='".$reciever_user_id."'"); 
            ...
            if($result_update)

             {
               ...
               $message=$send_result["name"]." sent you a follow request";

               $this->db->insert('sent_notification',$insert_notifiction);



               $deviceToken_get=$this->db->query("SELECT deviceToken from users where id='".$reciever_user_id."'")->row_array();
                ...
                echo json_encode(array("status"=>"1","message"=>"Send follow request"));
...

             {
                ...
                echo json_encode(array("status"=>"0","message"=>"Follow request not sent"));
...
$this->db->insert('sent_notification',$insert_notifiction);
    ...
    public function get_notification()
     ...
     $get_result_notification = $this->db->query("SELECT *,(select users.name from users where users.id=sent_notification.from_id) as name,(select users.profileimage from users where users.id=sent_notification.from_id) as profileimage from sent_notification where to_id='".$user_id."' ORDER BY id DESC")->result_array();
  ...
  "bio"=>@$bio
          ...
          if($update_profile)
...

           {
            ...
            echo json_encode(array("status"=>"1","data"=>$full_img_name,"message"=>"Profile has been updated successfully"));
...
$get_user_filling=$this->db->query("SELECT * FROM feeling  WHERE status='0' GROUP BY name ORDER BY reating DESC LIMIT 15")->result_array();
...
//3= read        
          ...
          try {
...

            {
              ...
              echo json_encode(array("status"=>"1","data"=>$chat_message_insert,"message"=>"Message Sent Successfully"));
