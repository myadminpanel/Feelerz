...
defined('BASEPATH') OR exit('No direct script access allowed');
 ...
 $email_data=$this->db->query("SELECT * FROM email_settings")->row_array();

    $this->load->library('email');
$this->data['send_by_email']  = $email_data["use_email"];
$config = array(
                'protocol'  => 'mail',
                'smtp_host' => $email_data["smtp_host"],
                'smtp_port' => 25,
                'smtpdebug' => 4,
                'smtp_user' => $email_data["use_email"],
                'smtp_pass' => $email_data["password"],
                'mailtype'  => 'html',
...
public function text_message_data_dsp()
{
  $user["deviceToken"]="CA4BEBF0D0097C75E0EDE6A7A6E364FE20EF31D3410CF26742BB0FA5654A734B";
  $message="test message";
  $this->notifitation_ios($user["deviceToken"],$message);
}

public function get_user_from_post_test_and_send_notic($message,$send_message,$user_id,$post_id)
{
  ...
  {
      // var_dump($message);
 $message_in_array=explode(" ",$message);
 // var_dump($message_in_array);
$get_username_in_array=[];
  for($i=0; $i<count($message_in_array); $i++)
  {
      if($this->startsWith( $message_in_array[$i], '@' ))
      {
        $username_in_array=explode("@",$message_in_array[$i]);
        $username_without_add_symbol=$username_in_array[1];
        
        array_push($get_username_in_array, $username_without_add_symbol);
      }
  }
  // var_dump($get_username_in_array);
  $user_data_get=$this->db->query("SELECT id,name,deviceToken FROM users WHERE userName IN ('".implode("','",($get_username_in_array))."')")->result_array();
    // var_dump($this->db->last_query());
    // var_dump($user_data_get);
 foreach($user_data_get as $user)
 {
  // var_dump($user["deviceToken"]);
$message_set=$send_message;
  ...
  $this->notifitation_ios($user["deviceToken"],$message_set);
 }
  }
     ...
     $data = json_decode($json);
      ...
      $this->db->update("users",array("otp"=>$otp_code),array("email"=>$email));
       ...
       //  $res=$this->load->view("email/send",array("email"=>@$email,"subject"=>'OTP FROM APP',"mess"=>$message));
          ...
          $this->email->to($email);
          $this->email->from($this->data['send_by_email'],'feelerapp');
          $this->email->subject('OTP FROM APP '.date("d-m-Y H:m:sa"));
          $this->email->message($message);
          $res=$this->email->send();  
         // var_dump(=$res);
        echo jsomp(array("status"=>"1","message"=>"We have sent an OTP to your email id '".@$email."'"));  
...
//           $this->db->update("users",array("userName"=>@$username[0]."".$user_id),array("id"=>$user_id));
    ...
    public function social_login()

      {
        ...
        {
          $datas=base64_decode($imageData);


          $imageName_only = time() . '.png';

          $full_img_name = "S_".$imageName_only;

          $imagePath = "assets/profile_img/".$full_img_name;
         // var_dump($imagePath);

          file_put_contents($imagePath, $datas);
        }
...

     {
       ...
       $this->db->insert("set_otp",$insert_otp_data);
        ...
        // $res=$this->load->view("email/send",array("email"=>@$email,"subject"=>'OTP FOR CHANGE PASSWORD FROM APP',"mess"=>$message));
            ...
            $this->email->to($email);
            $this->email->from($this->data['send_by_email'],'feelerapp');
            $this->email->subject('OTP FOR CHANGE PASSWORD FROM APP');
            $this->email->message($message);
            $this->email->send();
 ...
 //============ Api 02-04-2019 Start By Gopal =============//
...

   {
      ...
      // $post_textO=json_decode('"'.$post_textO.'"');
       //var_dump($message);
...
$get_user_data=$this->db->query("SELECT id,name FROM users WHERE id='".$user_id."' ")->row_array();

$message_send=@$get_user_data["name"]." has mentioned on post";
$this->get_user_from_post_test_and_send_notic($post_textO,$message_send,$user_id,$post_id);
// var_dump($this->db->last_query());
// $result=true;
         if($result)

         {
          ...
          $this->db->query("UPDATE feeling SET reating = reating + 250 WHERE id='".$feeling_id."'");
        ...
        if($get_data['hug'])

         {
          ...
          else
            {
              ...
              $get_data=1;
              $message=$user_name["name"]." give hugged on your '".@$get_data_for_notification["post_text"]."' post.";

              $message_2="hugged on your '".@$get_data_for_notification["post_text"]."' post.";
...
if(in_array($user_id, $repost_user_in_array))
{
          ...
          $this->db->query("DELETE FROM repost_child_parent WHERE id='".$get_child_id["id"]."'");
...
$message_send=@$commented_by["name"]." has mentioned on post comment.";
$this->get_user_from_post_test_and_send_notic($comment,$message_send,$user_id,$post_id);
  ...
  public function get_friend_posts()
   {
       ...
       $page_no=@$data->page_no;
        ...
        $limit=$page_no*50;
        $friend_list = $this->db->query("SELECT following,follower FROM users WHERE id='".$user_id."'")->row_array();
      

       $lets = $friend_list['following'];
       
       if (strlen($lets) > 0){
           
           if (strlen($friend_list['follower']) > 0){
               
               $lets = $lets .",". $friend_list['follower'];
           }
       ...
       $allPost_ids = "0"; 
             $resultss =$this->db->query("SELECT post_id FROM `repost_child_parent` WHERE user_id in(".$lets.") ")->result_array();

            if (count($resultss)>0) {
                
              $ids = array_map(function ($ar) {return $ar['post_id'];}, $resultss);
              $allPost_ids = implode(",", $ids);
           //   print_r($allPost_ids);
            }
        ...
        // var_dump("SELECT *,u.name as name ,u.profileimage as profileimage,(SELECT count(get_comment.id) from get_comment where get_comment.post_id=p.id) as comments_count FROM post p INNER JOIN users u on p.user_id = u.id WHERE p.user_id IN (".$friends_string.")or p.id in (".$allPost_ids.") ORDER BY p.date_set_by_system DESC LIMIT ".$limit.",50");
       // print_r("SELECT *,u.name as name ,u.profileimage as profileimage FROM post p INNER JOIN users u on p.user_id = u.id WHERE p.user_id IN (".$friends_string.")or p.id in (".$allPost_ids.") ORDER BY p.date_set_by_system DESC LIMIT ".$limit.",50");
         $all_post_list= $this->db->query("SELECT p.*,u.name as name ,u.profileimage as profileimage,(SELECT count(get_comment.id) from get_comment where get_comment.post_id=p.id) as comments_count FROM post p INNER JOIN users u on p.user_id = u.id WHERE p.user_id IN (".$lets.")or p.id in (".$allPost_ids.") ORDER BY p.date_set_by_system DESC LIMIT ".$limit.",50")->result_array();
        if ($all_post_list){
...
public function get_public_posts()
   {
       ...
       $limit=$page_no*50;
       // var_dump("SELECT p.*,u.name as name ,u.profileimage as profileimage,(SELECT count(get_comment.id) from get_comment where get_comment.post_id=p.id) as comments_count FROM post p INNER JOIN users u on p.user_id = u.id WHERE p.privacy IN ('Public') ORDER BY p.date_set_by_system DESC LIMIT ".$limit.",50");
      // var_dump("SELECT *,(select users.name from users where users.id=post.user_id) as name,(select users.profileimage from users where users.id=post.user_id) as profileimage,(SELECT count(get_comment.id) from get_comment where get_comment.post_id=post.id) as comments_count FROM post WHERE  privacy IN ('Public') ORDER BY post.date_set_by_system DESC LIMIT ".$limit.",50 ");
      $all_post_list= $this->db->query("SELECT p.*,u.name as name ,u.profileimage as profileimage,(SELECT count(get_comment.id) from get_comment where get_comment.post_id=p.id) as comments_count FROM post p INNER JOIN users u on p.user_id = u.id WHERE p.privacy IN ('Public') ORDER BY p.date_set_by_system DESC LIMIT ".$limit.",50")->result_array();
      if(@$all_post_list)
       {
         echo json_encode(array("status" => "1","data"=>$all_post_list));
       }  
...
public function get_feeling_posts()
 ...
 $friend_list = $this->db->query("SELECT following,block_by_me,block_by_other FROM users WHERE id='".$user_id."'")->row_array();
  ...
  if($friend_list['block_by_me']){

    $blockUsers = $friend_list['block_by_me'];
   }
   if($friend_list['block_by_other']){

    $blockUsers = "".$blockUsers.",".$friend_list['block_by_other']."";
   }
...

{
  ...
  //var_dump("SELECT id FROM feeling WHERE (name='".$feeling_name."' or nameO='".$feeling_name."')");
  
  $set_feeling_filter="feeling_id='".@$feeling['id']."'";
}

      // $friend_list = $this->db->query("SELECT following FROM users WHERE id='".$user_id."'")->row_array();
       //var_dump($friend_list['following']);
       //$friends_string = $friend_list['following'];
      // var_dump("SELECT * FROM post WHERE ".$set_feeling_filter." and user_id not IN (".$blockUsers.")  ORDER BY post.date_set_by_system DESC LIMIT ".$limit.",50 ");
      $all_post_list= $this->db->query("SELECT *,(select users.name from users where users.id=post.user_id) as name,(select users.profileimage from users where users.id=post.user_id) as profileimage,(SELECT count(get_comment.id) from get_comment where get_comment.post_id=post.id) as comments_count FROM post WHERE ".$set_feeling_filter." and user_id not IN (".$blockUsers.")  ORDER BY post.date_set_by_system DESC LIMIT ".$limit.",50  ")->result_array();
...
$get_user_filling=$this->db->query("SELECT * FROM feeling  WHERE status='0' ORDER BY reating DESC LIMIT 30")->result_array();
  ...
  echo json_encode(array("status" =>"1","data"=>$user_name,"message"=>"Generated User"),JSON_UNESCAPED_UNICODE);
   ...
   public function get_TagList() 
    {
        ...
        //  }
         ...
         $list_result = $this->db->query("SELECT name,profileimage,userName FROM users")->result_array();
   ...
   public function search_friendA()
    {
       ...
       // var_dump($search_text);
        ...
        elseif ($more=="p") {
          ...
          $limit=$page_no*50;
           $post_data=$this->db->query("SELECT a.id as user_id,a.name,a.profileimage,b.post_text,b.feeling FROM users as a ,post as b WHERE a.status='1' and a.otp='DONE' and (b.post_textO LIKE '%".$search_text."%' or b.post_text LIKE '%".$search_text."%') and b.user_id=a.id LIMIT ".$limit.",50")->result_array();
   ...
   public function search_Mention()
    ...
    public function delete_nofication()
    {
       ...
       $data = json_decode($json);
       $n_id=@$data->id;
       $this->db->query("DELETE FROM sent_notification WHERE id='".$n_id."'");
       echo json_encode(array("status"=>"1","message"=>"Comment delete successfully"));
